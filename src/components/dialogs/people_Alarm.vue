<template>
  <div class="peopleAlarmDialog_box1" v-if="dialogVisible">
    <el-dialog  :visible.sync="visibleBox" width="20rem"  append-to-body v-if="visibleBox">
      <div class="content_" v-if="checki == 1">
        <div class="camera">
                <player
                    :src="tvSrc"
                />
      ...
      <div class="content_"  v-if="checki == 2">
        <div class="camera" v-if="allData.images&&allData.images[0]">
         <img
             class="people-pic"
        ...
        <div class="tip">
          <img   class="tip_noimg" src="../../assets/imgs/common/noimg.png" alt="">
        </div>
        </div>
      </div>
      <div class="content_"  v-if="checki == 3">
            <div class="RelationChart" style="width: 100%;height: 100%; padding: 0.2rem 0.4rem;" >
              <div id="map" class="map" ref="map" style="height: 480px;width: 890px">
              </div>
           </div>
      </div>
    </el-dialog>
    <div class="peopleAlarmDialog">
      <div class="alarm_title">
        <div class="text">{{allData.alarmTypeName}} <span class="lable" v-if="allData.state">{{allData.state}}</span></div>   
        <div class="quitBtn" @click="dialogVisible=false">X</div>
      </div>
      <div class="alarm_content_people" v-if='type == "people"'>
        <div class="alarm_content_add">
          <div class="alarm_content_left">
            ...
            <div class="alarm_content_left_btn">
              <div class="video" @click='check(1)'>查看视频</div>
              <div class="img" @click='check(2)'>查看图片</div>
            </div>
          </div>
          <div class="alarm_content_right">
            <peopleInfoItem  :peopleInfo="peopleInfo"/>
             <div class="relationInfo" @click='check(3)'>关系人信息</div>
          </div>
          <div class="alarm_content_process">
              <div class="process_item" v-for="(item,index) in alarmHistoryContent" :key="index">
                ...
                <div class="content">
                  <div class="info" v-for="(v,i) in item.processContent.split('/n')" :key='i'>
                      {{v}}
                  </div>
                </div>
              </div>
      ...
      <div class="alarm_content_people" v-if='type == "vehicle"'>
        <div class="alarm_content_add">
          <div class="alarm_content_left">
            <div class="alarm_content_left_info">
              <div class="alarm_content_left_info_detail">
    ...
    getInfos(id){
      this.$get(apiNames['报警详情']+id).then(res=> {
        if (res.responseStatus.resultCode === 0) {  
                ...
                this.credentialNo = res.data.peopleInfo && res.data.peopleInfo.credentialNo
            } else {
              this.$newthost({
                  content: res.data.name
                })     
