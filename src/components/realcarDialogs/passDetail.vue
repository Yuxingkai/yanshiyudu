   ...
   <div class="passDetailDialog">
    ...
    <div class="passDetailDialog_content">
      <div class="passDetailDialog_content_info">
        <div class="passInfo">
            <div class="passInfo_record"> <span class="jus_name">车牌号</span>: {{info.plateNo}} <span class="passLabel"></span><span class="passBtn" :class="{isClickBtn: click}" @click="showPass">通行记录</span></div>
            <div class="peopleInfo_detail"><span class="jus_name">车主姓名</span>: {{info.peopleName ? info.peopleName : '陌生人'}}</div>
            <div class="peopleInfo_detail"><span class="jus_name">手机号码</span>: <span class="numStyle">{{info.phoneNo}}</span></div>
            <div class="peopleInfo_detail"><span class="jus_name">居住地址</span>: {{info.peopleName ? info.houseaddress : ''}}</div>
            <div class="peopleInfo_detail"><span class="jus_name">通行位置</span>: {{info.address}}</div>
            <div class="peopleInfo_detail"><span class="jus_name">通行时间</span>: <span class="numStyle">{{info.captureTime}}</span></div>
        </div>
        <div class="picInfos">
          <div class="picInfo">
            <div class="picInfo_img">
                <img
                  class="people-pic"
                  :src="info.bkgUrl"
                >
              ...
              <el-table
                ...
                <el-table-column
                 ...
                 <template slot-scope="scope">
                    <span>{{scope.row&&scope.row.inOutType == 1? '进' : '出'}}</span>
                  </template>
                </el-table-column>
                <el-table-column
                  width="120"
                  ...
                  label="通行位置">
                 <template slot-scope="scope">
                    <span style='overflow: hidden;text-overflow: ellipsis;white-space: nowrap;'>{{scope.row&&scope.row.address}}</span>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="captureTime"
...
export default {
  ...
  mounted () {
    this.$bus.on('carPassDetail', (item) => {
