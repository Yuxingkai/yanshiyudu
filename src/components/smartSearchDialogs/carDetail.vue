<template>
 <el-dialog  :visible.sync="dialogVisible" v-if='dialogVisible' :width="realWidth" append-to-body>
   <div class="car_Detail">
     <div class="car_Detail_title">
        车辆详情
     </div>   
    <div class="car_Detail_content">
      <div class="img_box">
          <div class="info_img"  v-if="info.platePic">
            <img
                  class="people-pic"
                  :src="faceUrlEx+info.platePic"
                >
        </div>
...
export default {
  ...
  mounted () {
    this.$bus.on('car_Detail', (item) => {
      this.dialogVisible = true
      this.info = item
      console.log(item,111111111111)
    let carStructData = JSON.parse(localStorage.getItem('carStructData'))
    
      let dataMap = new Map() 
      for (const item of carStructData) {
