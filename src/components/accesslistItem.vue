  ...
  <div class='prewarnItem_'>
    ...
    >
      ...
      <div style="display:flex;height:0.5rem;padding-top: 0.24rem;">
        ...
        <span
          class="prewarnItem_info_staus"
          style="flex:1;padding-right: 0.24rem;color:'#FFD99A'"
        >
          <span class="c_">{{typeCode[item.openType]}}</span></span>
      </div>
      <div style="display:flex;height:0.6rem">
        <span class="prewarnItem_info_laber">{{item.address}}</span>
        <span
          v-if="compareDate(item.openTime)=='今日' || compareDate(item.openTime) =='昨日'"
          class="prewarnItem_info_time"
          style="flex:1;text-align: right;padding-right: 0.24rem;"
        >
          <span style="margin-right:0.12rem">{{compareDate(item.openTime)}}</span>
          {{ formatDate(item.openTime,'HH:mm:ss')}}
        </span>
        <span
          v-else
          class="prewarnItem_info_time"
    ...
    >
      <div style="height:0.6rem">
        <span class="prewarnItem_info_name">{{item.peopleName ? item.peopleName : '陌生人' }}</span>
        <span class="prewarnItem_info_belong">{{item.labelNames}}</span>
      </div>
      <div style="display:flex;height:0.5rem;padding-top: 0.24rem;">
        <span class="prewarnItem_info_address">{{item.houseAddress}}</span>
      ...
      <div style="display:flex;height:0.6rem">
        <span class="prewarnItem_info_laber">{{item.address}}</span>
        <span
          v-if="compareDate(item.faceCaptureTime)=='今日' || compareDate(item.faceCaptureTime) =='昨日'"
          class="prewarnItem_info_time"
          style="flex:1;text-align: right;padding-right: 0.24rem;"
        >
          <span style="margin-right:0.12rem">{{compareDate(item.faceCaptureTime)}}</span>
          {{ formatDate(item.faceCaptureTime,'HH:mm:ss')}}
        </span>
        <span
          v-else
          class="prewarnItem_info_time"
    ...
    >
      <div style="height:0.6rem">
        <span class="prewarnItem_info_name">{{item.title ? item.title : '陌生人' }}</span>
      ...
      <div style="display:flex;height:0.5rem;padding-top: 0.24rem;">
        ...
        <span
          class="prewarnItem_info_staus"
          style="flex:1;padding-right: 0.24rem;"
          :style="{'color': item.state == '未处理' ? '#FF9999': (item.state == '已处理' ? '#00F9FC' : '#F8E71C')}"
        >{{item.state}}</span>
      </div>
      <div style="display:flex;height:0.6rem">
        <span class="prewarnItem_info_laber">{{item.description}}</span>
        <span
          v-if="compareDate(item.alarmTime)=='今日' || compareDate(item.alarmTime) =='昨日'"
          class="prewarnItem_info_time"
          style="flex:1;text-align: right;padding-right: 0.24rem;"
        >
...
import { faceUrl } from '../Utils/config.js'
export default {
  name: "prewarnItem",
  props: {
    item: {
